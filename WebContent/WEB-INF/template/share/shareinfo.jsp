...
<body class="share">
<div class="g-width Current_nav"><a href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>">首页</a> &gt; <a href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/share/new20.html">晒单分享</a> &gt; 晒单详请</div>
<div id="loadingPicBlock" class="share_box">
        <div class="share_box_left" id="DCMainLeft">
            <div class="share_main">
                ...
                <div class="share_goods">
                    <div class="share-get">
                        ...
                        <b class="sh_num">1000</b>
                    	</c:if>
                        <a href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/u/${shareInfoPro.userId }.html" target="_blank" class="fl-img"><img src="<%=ApplicationListenerImpl.sysConfigureJson.getImgUrl()%>${shareInfoPro.winUserFace }" /></a>
                        <div class="share-getinfo">
                            <p class="getinfo-name">幸运获得者：<a href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/u/${shareInfoPro.userId }.html" target="_blank" class="blue Fb">${shareInfoPro.winUserName }</a></p>
                            <p>总共<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>：<b class="orange">${shareInfoPro.buyNumberCount}</b>人次</p>
                    ...
                    <div class="share-Conduct">
                        <div class="arrow arrow_Rleft">
                            <em>◆</em><span>◆</span></div>
                        <a target="_blank" href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/lotteryDetail/${shareInfoPro.productId }.html" class="fl-img"><img border="0" alt="${shareInfoPro.productTitle }" src="<%=ApplicationListenerImpl.sysConfigureJson.getImgUrl()%>${shareInfoPro.productImg }" /></a>
                        <div class="share-getinfo">
                            <p class="getinfo-title"><a target="_blank" href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/lotteryDetail/${shareInfoPro.productId }.html" class="gray01">(第${shareInfoPro.productPeriod }期)${shareInfoPro.productName }</a></p>
           ...
           <input type="hidden" value="${resultCount }" id="hidReplyCount" name="hidReplyCount"/>
            ...
            <s:if test="user==null">
	            <img src="<%=ApplicationListenerImpl.sysConfigureJson.getImgUrl()%>/Images/defaultUserFace.png" name="imgUserPhoto" />
            </s:if>
            <s:else>
                ...
                <div class="reply_form">
                    <div class="reply_textbox">
                        ...
                        <div class="reply_login" id="notLogin">
                            请您<a href="javascript:;" name="replyLoginBtn" id="linklogin">登录</a>或<a href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/register/index.html?forward=rego">注册</a>后再回复评论
                        </div>                    
                    </div>
                </div>
        ...
        <!--晒单右侧-->
            <div class="Comment_share">
                ...
                <div class="New-single"><p class="New-single-time"><a href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/u/${ShareJSONs.userId }.html" target="_blank" class="blue">${ShareJSONs.userName }</a>${ShareJSONs.shareDate }</p>
                <p class="New-single-C"><a target="_blank" href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/shareShow/${ShareJSONs.uid }.html">${ShareJSONs.shareContent }</a></p>
                <div class="New-singleImg"><div class="arrow arrow_Rleft"><em>◆</em></div><a target="_blank" href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/shareShow/${ShareJSONs.uid }.html">
                <c:forEach items="${ShareJSONs.shareimageList}" var="shareimageLists" begin="0" end="3">
	                <img border="0" alt="${ShareJSONs.shareTitle }" src="<%=ApplicationListenerImpl.sysConfigureJson.getImgUrl()%>/UserPost/Small/${shareimageLists.images }" />
                </c:forEach>
                </a></div></div>
                </s:iterator>
            </div>
