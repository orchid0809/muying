...
//		page.setResultCount(resultCount);
	...
	public Pagination indexNewProductList(int pageNo, int pageSize) {
		StringBuffer hql = new StringBuffer("select pt.*,st.* from Product pt,Spellbuyproduct st where 1=1 and st.fkProductId=pt.productId  and st.spStatus <> 1  GROUP by pt.productId order by st.spellbuyStartDate desc");
//		StringBuffer sql = new StringBuffer("select count(*) from Spellbuyrecord");
		Pagination page = new Pagination();
		page.setPageNo(pageNo);
		page.setPageSize(pageSize);
...
//		page.setResultCount(resultCount);
	...
	public Pagination indexHotProductList(int pageNo, int pageSize) {
		StringBuffer hql = new StringBuffer("select pt.*,st.* from Product pt,Spellbuyproduct st where 1=1 and st.fkProductId=pt.productId  and st.spStatus <> 1  GROUP by pt.productId order by st.productPeriod desc");
//		StringBuffer sql = new StringBuffer("select count(*) from Spellbuyrecord");
		Pagination page = new Pagination();
		page.setPageNo(pageNo);
		page.setPageSize(pageSize);
