...
//		shareInfoPro.setShareimageList(shareImgList);
		 ...
		 * 最新晒单
		 */
		Pagination page = shareService.loadShareInfoByNew("new20",pageNo, pageSize);
		List<Object[]> pageList = (List<Object[]>) page.getList();
		ShareJSONList = new ArrayList<ShareInfoJSON>();
		for (int i = 0; i < pageList.size(); i++) {
			...
			ShareInfoJSON.setShareimageList(shareService.getShareimage(shareinfo.getUid().toString()));
