...
public class ShareServiceImpl implements ShareService{
	...
	public Pagination loadShareInfoByNew(String type, int pageNo, int pageSize) {
		StringBuffer hql = new StringBuffer("select so.*,ly.* from shareinfo so,latestlottery ly where so.productId = ly.spellbuyProductId and so.userId = ly.userId and so.status = 2 group by so.uid");
		StringBuffer sql = new StringBuffer("select count(DISTINCT(so.uid)) from shareinfo so,latestlottery ly where so.productId = ly.spellbuyProductId and so.userId = ly.userId and so.status = 2");
		if(type.equals("new20")){
			hql.append(" ORDER BY so.shareDate DESC");
		}
		if(type.equals("hot20")){
