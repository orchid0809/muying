						...
						try {
							if(userPayType==1){
								...
								if(user.getBalance()>=count){
									...
									user.setCommissionPoints(points+(count*ApplicationListenerImpl.sysConfigureJson.getBuyProduct()));
									...
									if(user.getInvite()!=null){
										...
										userCommission.setCommissionCount(tempCommissionCount+=(Double.parseDouble(String.valueOf(count))*ApplicationListenerImpl.sysConfigureJson.getCommission()));
										userCommission.setCommissionBalance(commissionBalance+=(Double.parseDouble(String.valueOf(count))*ApplicationListenerImpl.sysConfigureJson.getCommission()));
										userService.add(userCommission);
										commissionquery.setBuyMoney(Double.parseDouble(String.valueOf(count)));
										commissionquery.setCommission((Double.parseDouble(String.valueOf(count))*ApplicationListenerImpl.sysConfigureJson.getCommission()));
									...
									commissionpoints.setPay("+"+count*ApplicationListenerImpl.sysConfigureJson.getBuyProduct());
...
//									userService.add(user);
									if(user.getInvite()!=null){
										...
										commissionquery.setCommission((Double.parseDouble(String.valueOf(hidUseBalance))*ApplicationListenerImpl.sysConfigureJson.getCommission()));
										commissionquery.setDate(DateUtil.DateTimeToStr(new Date()));
										commissionquery.setDescription(ApplicationListenerImpl.sysConfigureJson.getShortName()+"商品编码("+productCart.getProductId()+")获得佣金");
										commissionquery.setInvitedId(user.getInvite());
										commissionquery.setToUserId(user.getUserId());
