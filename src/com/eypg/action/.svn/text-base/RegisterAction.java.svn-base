...
//		String ip = Struts2Utils.getRequest().getRemoteAddr();
	...
	public String mobilecheck() {
		if(MemCachedClientHelp.getIMemcachedCache().get(str)==null){
			Struts2Utils.render("text/html", "<script>alert(\"该手机号没有注册,请注册！\");window.location.href=\"/register/index.html\";</script>","encoding:UTF-8");
			return null;
		}else{
	...
	public void regSendMes() throws Exception{
		...
		if(isVerify.equals(Base64.getEncode(phone))){
			if(MemCachedClientHelp.getIMemcachedCache().get(Base64.getEncode(phone))==null){
				try {
					List<SendResultBean> sendList = Sampler.sendOnce(phone, "您本次的手机验证码是："+ran+"，请即时填写，祝您好运。");
					if(sendList != null){
		...
		try {
			if(MemCachedClientHelp.getIMemcachedCache().get(isVerify)!=null){
				if(MemCachedClientHelp.getIMemcachedCache().get(isVerify).equals(key)){
					...
					userPwd = (String) MemCachedClientHelp.getIMemcachedCache().get(phone);
					if(userPwd!=null && !userPwd.equals("")){
						user.setUserPwd(userPwd);
					}else{
